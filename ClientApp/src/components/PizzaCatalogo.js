...
export class PizzaCatalogo extends Component {
  ...
  handleClick() {
    ...
    const os = this.state.salsas
      .filter((salsa) => salsa.id == this.state.salsa)
      .pop();
    const ot = [];
    const ingre = this.state.ingredientes;
    console.log(ingre);
    this.state.toppings.forEach((it) => {
  ...
  delete = async (id) => {
    authService.getAccessToken().then(async (token) => {
      ...
      const response = await axios.delete("pizza/" + id, options);
    ...
    return (
      <div>
        <Container>
          ...
          <p>
            Este componente demuestra el uso de Fetch para ir a la API server
          </p>
            ...
            <tbody>
              {this.state.data.map((pizza) => (
                <tr key={pizza.id}>
                  ...
                  <td>{pizza.sauce}</td>
                  <td>
                    <Button color="primary" onClick={() => this.editar(pizza)}>
                      Edit
                    </Button>{" "}
                    <Button
                      color="primary"
                      onClick={() => this.delete(pizza.id)}
                    >
                      X
                    </Button>
                  </td>
        ...
        >
          <ModalHeader toggle={this.mitoogle}>Modal title</ModalHeader>
          <ModalBody>
            <Form>
              <FormGroup>
              ...
              <FormGroup>
                ...
                <Input
                  ...
                  onChange={this.handleChange}
                >
                  {this.state.salsas.map((salsa) => (
                    <option
                      value={salsa.id}
                      selected={
                        this.state.accion === 2 &&
                        this.state.pizzaE.sauce.id === salsa.id
                      }
                    >
                      {salsa.name}
                    </option>
              ...
              <FormGroup>
                <Label for="toppings">Ingredientes</Label>
                <Input
                  id="toppings"
                  multiple
                  name="toppings"
                  ...
                  onChange={this.handleChange}
                >
                  {this.state.ingredientes.map((topping) => (
                    <option
                      value={topping.id}
                      selected={
                        this.state.accion === 2 && this.marcarIngre(topping)
                      }
                    >
                      {topping.name}
                    </option>
          ...
          <ModalFooter>
            ...
            <Button onClick={function noRefCheck() {}}>Cancelar</Button>
          </ModalFooter>
        </Modal>
      </div>
    );
